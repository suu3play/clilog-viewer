...
def get_files():
    ...
    try:
        ...
        with sqlite3.connect(database.db_path) as conn:
            ...
            cursor = conn.execute('''
                SELECT filename, file_path, last_modified, 
                       COUNT(conversations.id) as message_count
                FROM log_files 
                LEFT JOIN conversations ON log_files.id = conversations.log_file_id
                GROUP BY log_files.id
...
def search_messages_by_date_range():
    ...
    try:
        ...
        # データベースから日付範囲検索実行
        results = database.get_conversations_in_range(start_date, end_date)
        ...
        for result in results:
            result['file_name'] = result.get('filename', 'unknown')
...
def get_stats():
    ...
    try:
        ...
        with sqlite3.connect(database.db_path) as conn:
            ...
            cursor = conn.execute('''
                SELECT 
                    COUNT(DISTINCT log_files.id) as file_count,
                    COUNT(conversations.id) as message_count
                FROM log_files 
                LEFT JOIN conversations ON log_files.id = conversations.log_file_id
            ''')
            ...
            # 最新ファイル情報を取得
            cursor = conn.execute('''
                SELECT filename, last_modified, COUNT(conversations.id) as message_count
                FROM log_files 
                LEFT JOIN conversations ON log_files.id = conversations.log_file_id
                GROUP BY log_files.id
                ORDER BY last_modified DESC
        ...
        stats = {
            'cached_files': counts['file_count'],
